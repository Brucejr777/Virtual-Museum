...
export function ExhibitionDetailPage() {
  ...
  const [activeWork, setActiveWork] = useState(artworks.find((work) => work.id === exhibition?.workIds[0])?.id ?? '');
  const activeArtwork = artworks.find((work) => work.id === activeWork);

  const relatedExhibitions = useMemo(() => exhibition?.relatedExhibitionIds.map((id) => exhibitions.find((item) => item.id === id)).filter(Boolean) ?? [], [exhibition]);
  const relatedArtists = useMemo(() => exhibition?.relatedArtistIds.map((id) => artists.find((item) => item.id === id)).filter(Boolean) ?? [], [exhibition]);
  const featuredWorks = useMemo<Artwork[]>(() => exhibition?.workIds.map((id) => artworks.find((work) => work.id === id)).filter((work): work is Artwork => Boolean(work)) ?? [], [exhibition]);

  if (!exhibition) {
    return <EmptyState title="Exhibition not found" description="This gallery may be between installations. Return to the exhibition program to keep exploring." action={<Link className="button button-dark" to="/exhibitions">View exhibitions</Link>} />;
  }
    ...
    <>
      ...
      <section className="section">
        <div className="section-inner">
          <SectionHeading eyebrow="Inside the gallery" title="Featured works" description="Choose an object to bring it closer. Each work opens a detailed record with materials, context, and provenance." />
          <div className="exhibition-gallery">
            <div className="exhibition-gallery-main">
              {activeArtwork ? (
                <Link className="exhibition-gallery-image" to={`/works/${activeArtwork.id}`}>
                  ...
                  <span className="gallery-zoom-label">View object record <span aria-hidden="true">→</span></span>
                </Link>
            ...
            <div className="exhibition-gallery-thumbs" role="tablist" aria-label="Featured works">
              {featuredWorks.map((work) => (
                <button className={activeWork === work.id ? 'is-active' : ''} key={work.id} type="button" onClick={() => setActiveWork(work.id)} role="tab" aria-selected={activeWork === work.id}>
                  <img src={work.image} alt="" />
                  <span><strong>{work.title}</strong><small>{work.artistId ? getArtist(work.artistId)?.name : 'Unknown maker'} · {work.date}</small></span>
                </button>
              ))}
            </div>
          </div>
          ...
          <div className="exhibition-timeline">
            {exhibition.timeline.map((item, index) => (
              <article className="exhibition-timeline-item" key={`${item.year}-${item.label}`}>
                ...
                <div><span>{item.year}</span><h3>{item.label}</h3><p>{item.text}</p></div>
              </article>
            ))}
          </div>
        </div>
      ...
      <section className="section section-tinted">
        <div className="section-inner">
          ...
          <div className="artist-grid artist-grid-three">{relatedArtists.map((artist) => artist && <ArtistCard key={artist.id} artist={artist} />)}</div>
        </div>
      </section>
      ...
      {relatedExhibitions.length > 0 && (
        <section className="section">
          <div className="section-inner">
            ...
            <div className="exhibition-grid exhibition-grid-three">{relatedExhibitions.map((item) => item && <ExhibitionCard key={item.id} exhibition={item} />)}</div>
          </div>
        </section>
      )}
    </>
