...
export function TimelinePage() {
  ...
  const events = useMemo(
    () => timeline.filter((event) => activeKind === 'All' || event.kind === activeKind),
    [activeKind],
  );

  // Keep the selected event inside the current filter.
  useEffect(() => {
  ...
  return (
    <>
      <PageHero
        ...
        title="A museum measured in moments"
        description="Follow the long conversation between materials, makers, and the worlds they inhabited."
        image="https://commons.wikimedia.org/wiki/Special:Redirect/file/KellsFol032vChristEnthroned.jpg?width=1280"
        imageAlt="An illuminated manuscript page"
      />
        ...
        <div className="section-inner">
          <div className="section-heading-row">
            <SectionHeading
              ...
              title="Choose a moment, follow a thread"
              description="Filter the timeline by artistic movement, event, creator, or object, then select a moment to read its story."
            />
          ...
          <div className="timeline-toolbar" role="tablist" aria-label="Timeline filters">
            {kinds.map((kind) => (
              ...
              >
                {kind === 'All' ? 'All moments' : kind[0].toUpperCase() + kind.slice(1)}
              </button>
            ))}
          </div>
          <div className="timeline-layout">
            <div className="timeline-track">
              {events.map((event, index) => (
                <button
                  className={`timeline-event${selected?.id === event.id ? ' is-active' : ''}`}
                  key={event.id}
                  type="button"
            ...
            <article className="timeline-detail">
              {selected ? (
                <>
                  ...
                  {selected.artworkId && (
                    <Link className="button button-outline-dark" to={`/works/${selected.artworkId}`}>
                      View related object <span aria-hidden="true">→</span>
                    </Link>
                  )}
                </>
              ) : (
          ...
          {events.length === 0 && (
            <EmptyState
              ...
              description="Try another timeline filter to continue through history."
            />
          )}
        </div>
      </section>
